...
import { wpWatchProcessCompilation, wpWatchProcessDone } from "../util/messages";
    ...
    apply(compiler: WebpackCompiler) {
        compiler.plugin("compilation", () => {
            this.options.publish(
                wpWatchProcessCompilation(this.options.publisher)
            );
        });
        compiler.plugin("done", (stats: WebpackStats) => {
            this.options.publish(
                wpWatchProcessDone(this.options.publisher, stats)
            );
        });
    }
}
